module SDRAM_command (
    input  logic                     CLK,        // System Clock
    input  logic                     RESET_N,    // System Reset
    input  logic [`ASIZE-1:0]        SADDR,      // Address
    input  logic                     NOP,        // Decoded NOP command
    input  logic                     READA,      // Decoded READA command
    input  logic                     WRITEA,     // Decoded WRITEA command
    input  logic                     REFRESH,    // Decoded REFRESH command
    input  logic                     PRECHARGE,  // Decoded PRECHARGE command
    input  logic                     LOAD_MODE,  // Decoded LOAD_MODE command
    input  logic                     REF_REQ,    // Hidden refresh request
    input  logic                     INIT_REQ,   // Hidden initial request
    input  logic                     PM_STOP,    // Page mode stop
    input  logic                     PM_DONE,    // Page mode done
    output logic                     REF_ACK,    // Refresh request acknowledge
    output logic                     CM_ACK,     // Command acknowledge
    output logic                     OE,         // OE signal for data path module
    ...
    output logic                     RAS_N,      // SDRAM RAS
    output logic                     CAS_N,      // SDRAM CAS
    output logic                     WE_N        // SDRAM WE_N
);
